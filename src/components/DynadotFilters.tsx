...
import { Search, X, HelpCircle, Check } from 'lucide-react';
...
interface Props {
    ...
    patternHelpOpen: boolean;
    setPatternHelpOpen: (open: boolean) => void;
    patternCount: number;
}
        ...
        <div className="px-5 py-4 bg-slate-50/50 border-b border-slate-100 space-y-4">
            <div className="flex flex-wrap gap-6 items-start">
                ...
                <div className="space-y-2 flex-1 min-w-[280px]">
                    <div className="flex items-center justify-between">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Filter by TLDs</label>
                ...
                <div className="flex gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Max Price (USD)</label>
                        <input
                            type="number"
                            min={0}
                    ...
                    <div className="space-y-1">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Min Bids</label>
                        <input
                            type="number"
                            min={0}
            ...
            <div className="flex flex-wrap gap-6 items-start">
                ...
                <div className="space-y-1 flex-1 min-w-[200px]">
                    <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Search Domain</label>
                ...
                <div className="space-y-1 flex-1 min-w-[280px]">
                    <div className="flex items-center gap-1.5">
                        <label className="text-[10px] font-semibold text-slate-500 uppercase tracking-wide">Domain Name Pattern</label>
                        <button
                            type="button"
                            onClick={() => setPatternHelpOpen(!patternHelpOpen)}
                    ...
                    />
                    {patternHelpOpen && (
                        <div className="mt-2 p-3 bg-indigo-50 border border-indigo-100 rounded-xl text-[11px] text-slate-600 leading-relaxed space-y-2">
                            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
                                {[
                                    ...
                                    ['-', 'Hyphen'],
                                    ['*', 'Anything (wildcard)'],
                                    ['A / B / H / K', '2 / 3 / 4 / 5 same letters'],
                                    ['D / E / F / G', '2 / 3 / 4 / 5 same digits'],
                                ].map(([sym, desc]) => (
                                    <div key={sym} className="flex items-baseline gap-1.5 col-span-1">
                                        <code className="font-bold text-indigo-600 text-[11px]">{sym}</code>
                                        <span className="text-slate-500">{desc}</span>
                                    </div>
                                ))}
                            </div>
                            <p className="text-slate-400 border-t border-indigo-100 pt-1.5">
                                Separate patterns with spaces (logical OR). Matches only the SLD.
                            </p>
                        </div>
                    )}
                </div>
