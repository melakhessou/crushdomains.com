...
export type AuctionsResult =
    | { status: 'success'; auctions: Auction[]; generatedAt: string }
...
export async function getAuctions(forceRefresh = false): Promise<AuctionsResult> {
    const apiKey = process.env.DYNA_DOT_API_KEY;
    if (!apiKey) {
        return { status: 'error', message: 'API key missing. Add DYNA_DOT_API_KEY to your environment variables.', code: 500 };
    }
    ...
    if (fatalFetchError && allRaw.length === 0) {
        return { status: 'error', message: `Failed to reach Dynadot API: ${fatalFetchError}`, code: 502 };
    }
    ...
    if (dynadotData?.status !== 'success' && allRaw.length === 0) {
        ...
        return {
            ...
            code: isRateLimit ? 429 : 502
        };
    }

    // Normalize and Filter
    const auctions: Auction[] = allRaw.map((item) => {
    ...
    if (auctions.length > 0) {
        await setToKV(MASTER_CACHE_KEY, JSON.stringify({ auctions, fetchedAt: generatedAt }), CACHE_TTL);
    }
